...
class GameState:
    def __init__(self):
        ...
        self.move_log = [] # list to save and show the moves that were played
        self.white_king_location = (7, 4)
        self.black_king_location = (0, 4)
        self.checkmate = False
        self.stalemate = False
        self.in_check = False
        self.pins = [] #if there was a piece coves the king
        self.checks = []
        self.enpassant_possible = ()  # coordinates for the square where en-passant capture is possible
        self.enpassant_possible_log = [self.enpassant_possible]
        # constructor all true if any king or castle then its flag will be false
        # tapieet alhara8
    ...
    #row from 1-8 & columns from a-h
    #moves is the list that stores all possible moves for soldier
    def getPawnMoves(self, row, col, moves):
        """
        Get all the pawn moves for the pawn located at row, col and add the moves to the list.
        """
        #first we set the pin with false
        piece_pinned = False
        pin_direction = ()
        #size of the pin list (0-7)
        #checks if this piece is pinned or not
        #like a soldier stands in front of the king and that soldier piece saves the king so cannot move or eat another piece
        ...
        if self.white_to_move:
            #flag for moving to white
            move_amount = -1
            #the row of soldiers (W)
            start_row = 6
            enemy_color = "b"
            king_row, king_col = self.white_king_location
        else:
            # flag for moving to black
            move_amount = 1
            # the row of soldiers (B)
            start_row = 1
            enemy_color = "w"
            king_row, king_col = self.black_king_location
        if self.board[row + move_amount][col] == "--":  #check if the square is empty or not
            if not piece_pinned or pin_direction == (move_amount, 0):   #if not pinned move
                moves.append(Move((row, col), (row + move_amount, col), self.board))  #move then add to the Moves list
                if row == start_row and self.board[row + 2 * move_amount][col] == "--":  # soldier moves one or two at the first
                    moves.append(Move((row, col), (row + 2 * move_amount, col), self.board))
        if col - 1 >= 0:  #check for the soldier if it can eat for the left
            if not piece_pinned or pin_direction == (move_amount, -1):
                ...
                if (row + move_amount, col - 1) == self.enpassant_possible:
                    ...
                    if king_row == row:
                        if king_col < col:  # king of the opponent is left of the pawn
        ...
        if col + 1 <= 7:  #check for the soldier if it can eat for the right
            if not piece_pinned or pin_direction == (move_amount, +1):
                if self.board[row + move_amount][col + 1][0] == enemy_color:
                    moves.append(Move((row, col), (row + move_amount, col + 1), self.board))
                if (row + move_amount, col + 1) == self.enpassant_possible:
                    attacking_piece = blocking_piece = False  #it should all still false till the end of condition
                    if king_row == row:
                        ...
                        for i in outside_range:
                            ...
                            if square[0] == enemy_color and (square[1] == "R" or square[1] == "Q"):
                                ...
                                blocking_piece = True
                    if not attacking_piece or blocking_piece:
                        moves.append(Move((row, col), (row + move_amount, col + 1), self.board, is_enpassant_move=True)) #troy
